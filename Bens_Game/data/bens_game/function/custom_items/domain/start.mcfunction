say start
execute as @a[tag=domain_user] run tag @e[distance=1..4,tag=!domain_user] add domain_target

#summon a marker to user position: user_marker                Marker:1b,Invisible:1b,
execute as @a[tag=domain_user] run summon armor_stand ~ ~ ~ {CustomName:"user_marker"}

#summon a marker to target position: target_marker
execute at @e[tag=domain_target] run summon armor_stand ~ ~ ~ {CustomName:"target_marker"}
...
schedule function bens_game:domain/display_name 60t

schedule function bens_game:domain/effects 80t
schedule function bens_game:domain/end 400t