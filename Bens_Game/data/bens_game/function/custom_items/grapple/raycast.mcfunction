...
execute as @e[distance=..1.0,type=!item,type=!experience_orb,tag=!grapple_caster,limit=1,sort=nearest] run launch @s toward @a[tag=grapple_caster,limit=1] 1.5
...
execute if entity @e[type=marker,tag=grapple_target,limit=1] as @a[tag=grapple_caster,limit=1] at @s run launch @s toward @e[type=marker,tag=grapple_target,limit=1] 1.5
